...
const gatewayLogos: Record<string, string> = {
    bridgerpay: "https://cdn.brokeret.com/crm-assets/admin/pg/bridgerpay.webp",
    paypal: "https://cdn.brokeret.com/crm-assets/admin/pg/paypal.png",
    stripe: "https://cdn.brokeret.com/crm-assets/admin/pg/stripe.webp",
    skrill: "https://cdn.brokeret.com/crm-assets/admin/pg/skrill.png",
};

const initialMethods = [
    { provider: "BridgerPay", logo: "https://cdn.brokeret.com/crm-assets/admin/pg/bridgerpay.webp", title: "BridgerPay", status: "Active", processing: "Instant", fee: "0", limits: "5 - USD", branches: "N/A" },
];
...
export default function AutomaticWithdraw({ openAddIntent, onAddIntentConsumed }: Props) {
    ...
    const resetForm = () => {
        setFormData({
            ...
            conversionRate: "", currencySymbol: "", minWithdraw: "", maxWithdraw: "",
    ...
    const handleUpdate = (item: any, index: number) => {
        ...
        setFormData({
            ...
            conversionRate: "",
            currencySymbol: "",
            minWithdraw: item.limits?.split(" - ")[0] || "",
            maxWithdraw: item.limits?.split(" - ")[1]?.split(" ")[0] || "",
    ...
    const handleSave = () => {
        ...
        const payload = {
            ...
            limits: `${formData.minWithdraw} - ${formData.maxWithdraw}`,
                    ...
                    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
                        <Card className="w-full max-w-5xl bg-card max-h-[90vh] overflow-hidden flex flex-col">
                            <div className="flex justify-between items-center px-6 py-4 border-b border-border flex-shrink-0">
                                <h2 className="text-xl font-semibold">{editIndex !== null ? "Update Withdraw Method" : "Add New Withdraw Method"}</h2>
                            ...
                            <div className="p-6 overflow-y-auto flex-1">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <SelectField
                                        ...
                                        options={[
                                            { value: "bridgerpay", label: "BridgerPay" },
                                    ...
                                    <SelectField
                                        ...
                                        tooltip="The currency used by the selected gateway (e.g., USD, EUR)"
                                    />
                                    <InputField
                                        label="Name"
                                        name="name"
                                        ...
                                        tooltip="Enter a user-friendly name for this method"
                                    />

                                    <InputField label="Currency Symbol" name="currencySymbol" value={formData.currencySymbol} onChange={handleInputChange} disabled tooltip="The symbol representing the transaction currency (e.g., $, €, ₿)" placeholder="$" />

                                    <div>
                                        <StatusToggle label="Manual Conversion Rate" status={manualRate} onChange={setManualRate} tooltip="Enable to set manually coversion rate" />
                                    ...
                                    <InputField label="Conversion Rate" name="conversionRate" value={formData.conversionRate} onChange={handleInputChange} placeholder="1" disabled={manualRate === "Disabled"} tooltip="Define the conversion from 1 AUD to the target currency" prefix="1 USD =" suffix="USD" />
                                    <InputField
                                        label="Charges"
                                        name="charges"
                                        type="number"
                                        ...
                                        suffix={
                                            <select name="chargeType" value={formData.chargeType} onChange={(e) => handleSelectChange("chargeType", e.target.value)} className="outline-none bg-transparent">
                                    ...
                                    <InputField label="Minimum Withdraw" name="minWithdraw" type="number" value={formData.minWithdraw} onChange={handleInputChange} placeholder="5" tooltip="The minimum withdraw allowed using this method" suffix="USD" />
                                    <InputField label="Maximum Withdraw" name="maxWithdraw" type="number" value={formData.maxWithdraw} onChange={handleInputChange} placeholder="10000" tooltip="The minimum withdraw allowed using this method" suffix="USD" />

                                    <div className="col-span-2">
                                        <SelectField
                                            label="Select Countries Authorized to Use"
                                            options={[{ value: "all", label: "All Countries" }, { value: "us", label: "United States" }]}
                                    ...
                                    <div className="col-span-2">
                                        <SelectField
                                            ...
                                            options={[{ value: "main", label: "Main Branch" }, { value: "branch1", label: "Branch 1" }]}
                                            values={formData.branches}
                                            onValuesChange={(values) => handleMultiSelectChange("branches", values)}
                                            placeholder="Select an Option"
                                            tooltip="Select branches where this withdraw method will be available"
                                            isMulti
                                        />
                                        <p className="text-xs text-muted-foreground mt-2">
